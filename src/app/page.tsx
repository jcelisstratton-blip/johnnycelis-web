...
export default function Home() {
  ...
  const handleRunAiPrompt = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setIaLog(prev => [...prev, { role: "ai", text: "Procesando variables operativas... Transfiriendo requerimiento al equipo de ingeniería para diseño de ecosistema. Nuestro equipo se pondrá en contacto pronto." }]);
    }, 1800);
  };
    ...
    <main style={{ backgroundColor: '#000', color: '#fff', minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif', margin: 0, padding: 0, overflowX: 'hidden' }}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        * { box-sizing: border-box; scroll-behavior: smooth; }
        ...
        .link-explore { display: inline-flex; align-items: center; gap: 10px; color: #fff; text-decoration: none; font-weight: 900; font-size: 1.1rem; text-transform: uppercase; letter-spacing: 2px; transition: 0.4s; padding-bottom: 5px; border-bottom: 1px solid transparent; cursor: pointer; }
        ...
        .marquee-text-filled { font-size: clamp(1.2rem, 3vw, 1.5rem); font-weight: 900; color: #fff; text-shadow: 0 0 15px rgba(255,255,255,0.4); text-transform: uppercase; letter-spacing: 2px; }
        .marquee-text-stroke { font-size: clamp(1.2rem, 3vw, 1.5rem); font-weight: 900; color: transparent; -webkit-text-stroke: 1px ${electricPurple}; text-transform: uppercase; letter-spacing: 2px; }
        ...
        /* AI TERMINAL UI */
        .ai-terminal { background: #050505; border: 1px solid rgba(157,0,255,0.4); border-radius: 12px; font-family: monospace; display: flex; flex-direction: column; height: 400px; position: relative; box-shadow: 0 10px 40px rgba(0,0,0,0.8); }
        .ai-header { background: #000; padding: 15px 20px; border-bottom: 1px solid rgba(157,0,255,0.2); display: flex; justify-content: space-between; align-items: center; font-size: 0.85rem; color: #888; }
        .ai-status-dot { display: inline-block; width: 8px; height: 8px; background: #25D366; border-radius: 50%; box-shadow: 0 0 10px #25D366; margin-right: 10px; }
        .ai-log { flex: 1; padding: 20px; overflow-y: auto; display: flex; flex-direction: column; gap: 15px; color: #bbb; font-size: 0.95rem; }
        ...
        .ai-prompt { display: flex; padding: 15px; background: #000; border-top: 1px solid rgba(157,0,255,0.2); }
        .ai-input { flex: 1; background: transparent; border: none; color: #fff; font-family: inherit; font-size: 1rem; outline: none; }
        .ai-submit { background: transparent; color: ${electricPurple}; border: none; font-weight: bold; cursor: pointer; text-transform: uppercase; font-family: inherit; }
          ...
          {activePost && (
            <>
              ...
              <div style={{ marginTop: '50px', textAlign: 'center' }}>
                <a href="https://calendar.app.google/wCHwj3MuUxr4EUEp6" target="_blank" rel="noopener noreferrer" className="btn-glow">Analizar mi caso</a>
              </div>
            </>
          )}
        </div>
      ...
      {/* BANDAS MARQUEE */}
      <div style={{ background: '#030303', overflow: 'hidden', padding: '20px 0', borderTop: '1px solid #111' }}>
      ...
      {/* SECCIÓN ASESORÍA INTELIGENTE (AI TERMINAL) */}
      <section id="asesoria" style={{ padding: 'clamp(60px, 10vw, 120px) 5%', background: '#050505', borderTop: '1px solid #111', borderBottom: '1px solid #111' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', flexWrap: 'wrap', gap: 'clamp(40px, 8vw, 60px)', alignItems: 'center' }}>
          <div style={{ flex: '1 1 400px' }}>
            ...
            <h2 style={{ fontSize: 'clamp(2.5rem, 4vw, 3.5rem)', fontWeight: 900, textTransform: 'uppercase', lineHeight: 1.1, marginBottom: '25px' }}>
              Motor <br/><span style={{ color: electricPurple }}>Analítico</span>
            </h2>
            <p style={{ color: '#aaa', fontSize: '1.1rem', lineHeight: 1.7, marginBottom: '30px' }}>
              Ingresa tu problema operativo en nuestro terminal. La IA evaluará el cuello de botella y registrará la solicitud para que nuestro equipo diseñe una solución automatizada.
            </p>
          </div>
          ...
          <div style={{ flex: '1 1 500px', minWidth: '300px' }}>
            <div className="ai-terminal">
              <div className="ai-header">
                <div><span className="ai-status-dot"></span> STATUS: ONLINE</div>
                <div>v.2.0.4</div>
              </div>
              ...
              <form onSubmit={handleRunAiPrompt} className="ai-prompt">
                <span style={{ color: electricPurple, marginRight: '10px', fontWeight: 'bold' }}>$</span>
                <input 
                  type="text" 
                  className="ai-input" 
                  placeholder="Describe tu cuello de botella aquí..." 
                  value={iaInput} 
                  onChange={(e) => setIaInput(e.target.value)}
                  disabled={isProcessing}
                />
                <button type="submit" className="ai-submit" disabled={isProcessing}>RUN</button>
              </form>
            </div>
          </div>
        </div>
            ...
            <div style={{ flex: '2 1 600px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '30px' }}>
              {blogPosts.map((post, i) => (
                <div key={i} className="glass-card" style={{ padding: '30px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  ...
                  <div onClick={() => setActivePost(post)} className="link-explore" style={{ fontSize: '0.9rem', marginTop: 'auto' }}>Leer artículo <span className="arrow">→</span></div>
                </div>
              ))}
            </div>
