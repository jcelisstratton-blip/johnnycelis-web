    ...
    <main style={{ backgroundColor: 'black', color: 'white', minHeight: '100vh', fontFamily: 'sans-serif', margin: 0, overflowX: 'hidden' }}>
      ...
      <nav style={{ position: 'fixed', top: 0, width: '100%', zIndex: 1000, padding: '20px 50px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.95)', borderBottom: `1px solid ${electricPurple}33` }}>
        ...
        <div style={{ display: 'flex', gap: '25px', fontSize: '10px', fontWeight: 'bold', letterSpacing: '2px' }}>
          ...
          <a href="#agendar" style={{ backgroundColor: electricPurple, color: 'white', padding: '10px 20px', textDecoration: 'none' }} className="electric-btn">BOOK A CALL</a>
        </div>
      </nav>

      {/* HERO */}
      <section style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', textAlign: 'center', padding: '100px 20px' }}>
        ...
        <p style={{ maxWidth: '600px', color: '#888', fontSize: '1.1rem', margin: '40px 0', lineHeight: '1.6' }}>
          Escalamos agencias mediante agentes de voz IA y procesos autónomos. Eficiencia de élite para el mundo real.
        </p>
        <a href="#agendar" style={{ backgroundColor: electricPurple, color: 'white', padding: '20px 45px', fontWeight: 900, textDecoration: 'none' }} className="electric-btn">RESERVAR LLAMADA</a>
      </section>
      ...
      <section id="servicios" style={{ padding: '120px 20px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px' }}>
          {[
            { t: "Inbound AI", d: "Sustituye recepcionistas por agentes de voz que atienden y agendan 24/7." },
            { t: "Outbound AI", d: "Prospección masiva con voces humanas clonadas para cierre de leads." },
            { t: "n8n Architecture", d: "Automatización total de flujos de trabajo en tus herramientas actuales." }
          ].map((s, i) => (
            <div key={i} style={{ padding: '50px', border: '1px solid #111', background: '#080808' }}>
        ...
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2px', background: '#222' }}>
            <div style={{ background: '#000', padding: '40px' }}>
              <h4 style={{ color: electricPurple, fontWeight: 900 }}>STRATT-ON</h4>
              <p style={{ fontSize: '14px', color: '#aaa' }}>✓ 24/7 Sin descanso<br/>✓ Ahorro del 80%<br/>✓ Escalabilidad Instantánea</p>
            </div>
            <div style={{ background: '#000', padding: '40px' }}>
              <h4 style={{ color: '#444', fontWeight: 900 }}>TRADICIONAL</h4>
              <p style={{ fontSize: '14px', color: '#444' }}>✗ Horarios Limitados<br/>✗ Altos Costos Fijos<br/>✗ Difícil de Escalar</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="casos" style={{ padding: '120px 20px', maxWidth: '1200px', margin: '0 auto' }}>
        <h2 style={{ fontSize: '3rem', fontWeight: 900, fontStyle: 'italic', marginBottom: '60px' }}>RESULTADOS</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px' }}>
          {[
            { n: "The Luxe Week", m: "+$400k Mensuales", t: "Implementamos agentes de voz para reservas globales eliminando el cuello de botella." },
            { n: "Jili Properties", m: "90% Automatizado", t: "Todas las llamadas de mantenimiento ahora son filtradas por nuestra IA." }
          ].map((test, i) => (
            <div key={i} style={{ padding: '40px', borderLeft: `4px solid ${electricPurple}`, background: '#080808' }}>
              <p style={{ fontStyle: 'italic', color: '#888' }}>"{test.t}"</p>
              <h4 style={{ margin: '20px 0 5px 0' }}>{test.n}</h4>
              <span style={{ fontSize: '12px', color: electricPurple, fontWeight: 'bold' }}>{test.m}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="roi" style={{ padding: '120px 20px', background: 'white', color: 'black' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '80px', alignItems: 'center' }}>
          <div>
            <h2 style={{ fontSize: '4rem', fontWeight: 900, fontStyle: 'italic', lineHeight: 0.9 }}>SAVINGS<br /><span style={{ color: electricPurple }}>CALCULATOR</span></h2>
            <div style={{ marginTop: '50px' }}>
              <p style={{ fontSize: '12px', fontWeight: 'bold' }}>HORAS MANUALES: {hours}H / SEMANA</p>
              <input type="range" min="5" max="150" value={hours} onChange={e => setHours(parseInt(e.target.value))} style={{ width: '100%' }} />
            </div>
          </div>
          <div style={{ background: '#000', color: 'white', padding: '60px', textAlign: 'center' }}>
            <span style={{ fontSize: '10px', color: electricPurple, letterSpacing: '3px' }}>AHORRO MENSUAL</span>
            <div style={{ fontSize: '5rem', fontWeight: 900, fontStyle: 'italic', margin: '15px 0' }}>${savings.toLocaleString()}</div>
            <div style={{ borderTop: '1px solid #222', paddingTop: '20px' }}>
              <span style={{ fontSize: '10px', opacity: 0.5 }}>ANUAL PROYECTADO:</span>
      ...
      <section style={{ padding: '120px 20px', maxWidth: '800px', margin: '0 auto' }}>
        <h2 style={{ fontSize: '2.5rem', fontWeight: 900, fontStyle: 'italic', marginBottom: '50px' }}>FAQ</h2>
        {[{ q: "¿Se integra con mi CRM?", a: "Sí, conectamos vía API a HubSpot, Salesforce y más." }].map((f, i) => (
          <div key={i} style={{ border: '1px solid #111', background: '#080808', padding: '30px' }}>
            <h5 style={{ color: electricPurple, margin: '0 0 10px 0' }}>{f.q}</h5>
      ...
      <footer id="agendar" style={{ padding: '100px 20px', textAlign: 'center', borderTop: '1px solid #111' }}>
        <h2 style={{ fontSize: '2.5rem', fontWeight: 900, marginBottom: '40px' }}>READY?</h2>
